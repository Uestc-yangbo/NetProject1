...
int FindFrame(char *str){
	...
	rep(i,beginnum,endnum){//将找出来的帧拷贝到接收帧中
		RecFrame[i]=str[i];
	}
} 
...
//帧封装、解封模块
namespace FRAME{
	int frame(char *str){//帧封装
        int target=FindTarget(str);
        printf("target of str为：%d\n",target);
		char Str[10];
...
int main(){
    ...
    int PhyPort1=14102,NetPort=14502,AppPort=14302,PhyPort2=15102,YouPort=16102;
    ...
    printf("请输入物理层1端口号：\n");
    printf("%d\n",PhyPort1);
    sockaddr_in PhyAddr1;
    memset(&PhyAddr1, 0, sizeof(PhyAddr1));
    int PhyLen1=sizeof(PhyAddr1);
    PhyAddr1.sin_family = PF_INET;
    PhyAddr1.sin_addr.s_addr = inet_addr("127.0.0.1"); 
    PhyAddr1.sin_port = htons(PhyPort1);

	/*---------------------分割线-------------------*/

	//测试与物理层的联通性
	sendto(sock,"connect",8,0,(sockaddr*)&PhyAddr1,sizeof(PhyAddr1));
	int retval = recvfrom(sock,buffer,MAX_BUFFER_SIZE,0,(sockaddr*)&PhyAddr1,&PhyLen1);
	...
	printf("请输入物理层2端口号：\n");
    printf("%d\n",PhyPort2);
    sockaddr_in PhyAddr2;
    memset(&PhyAddr2, 0, sizeof(PhyAddr2));
    int PhyLen2=sizeof(PhyAddr2);
    PhyAddr2.sin_family = PF_INET;
    PhyAddr2.sin_addr.s_addr = inet_addr("127.0.0.1"); 
    PhyAddr2.sin_port = htons(PhyPort2);

	/*---------------------分割线-------------------*/

	//测试与物理层的联通性
	sendto(sock,"connect",8,0,(sockaddr*)&PhyAddr2,sizeof(PhyAddr2));
	retval = recvfrom(sock,buffer,MAX_BUFFER_SIZE,0,(sockaddr*)&PhyAddr2,&PhyLen2);
	if(retval == SOCKET_ERROR){
		printf("接口API关联失败\n");
		return 0;
	}
    ...
    while(1){
        ...
        if(IsSend[0]=='1'){
			...
			if(!strcmp(buf,mybuf)){//对比目的端口号，判断是否为群发消息
				printf("该信息为群发消息！\n");
				itoa(PhyPort1,SourcePort,10);//将本地端口号转化为字符串类型
				memset(buf, 0, sizeof(buf));
			    ...
			    strcat(Buffer,end);
			    sendto(sock, Buffer, strlen(Buffer), 0, (SOCKADDR*)&PhyAddr1, sizeof(PhyAddr1));//
			    memset(buffer,0,sizeof(buffer));
                retval=recvfrom(sock, buffer, MAX_BUFFER_SIZE, 0, (SOCKADDR*)&PhyAddr1, &PhyLen1);
			    if(retval){
				    printf("%s\n",buffer);//连接反馈信息
				...
				printf("该信息为点到点信息！\n");
				int TargetPort=atoi(buf);
			    memset(buf, 0, sizeof(buf));
                itoa(PhyPort1,SourcePort,10);//将端口号转化为字符串类型
			    memset(buf, 0, sizeof(buf));
			    strcat(buf,SourcePort);
			    strcat(buf,buffer);
			    printf("处理后的信息为：%s\n",buf);            
		        ...
		        printf("转码成功！比特流为:%s\n",sendbuffer);
				uint Num[MAX_BUFFER_SIZE],Len=1;
				if(TargetPort==YouPort){
					printf("对方可以直接相连，不经过交换机\n");
					while(1){	//每隔0.5秒发送一次请求信息	
				        printf("正在请求发送.........\n"); 
                        sendto(sock, "11111111", 8, 0, (SOCKADDR*)&PhyAddr2, sizeof(PhyAddr2));
                        memset(buffer, 0, sizeof(buffer));
                        recvfrom(sock, buffer, MAX_BUFFER_SIZE, 0, (SOCKADDR*)&PhyAddr2, &PhyLen2);
                        if(buffer[1]=='0'){
        	                printf("对方忙.....将稍后发送\n");
			                Sleep(500); 
		                }
			        ...
			        printf("发送的长度信息为：%s\n",buf);
			        printf("一共要发送%d帧\n",strlen(sendbuffer)/8);
			        sendto(sock, buf, strlen(buf), 0, (SOCKADDR*)&PhyAddr2, sizeof(PhyAddr2));
			        memset(buf,0,sizeof(buf));
			        recvfrom(sock, buf,MAX_BUFFER_SIZE, 0, (SOCKADDR*)&PhyAddr2, &PhyLen2);
			        if(buf[0]=='1'){
				        printf("对方接收成功！\n");
			        }
			        else{
                    ...
                    rep(i,0,strlen(sendbuffer)){//成帧发送
        	            ...
        	            if(cunt>=8){
        	                ...
        	                while(1){
        	                    ...
        	                    sendto(sock, TFrame, strlen(TFrame), 0, (SOCKADDR*)&PhyAddr2, sizeof(PhyAddr2));
        	                    printf("发送完毕，正在等待确认.......\n");
        	                    recvfrom(sock, checkbuffer,MAX_BUFFER_SIZE, 0, (SOCKADDR*)&PhyAddr2, &PhyLen2);
        	                    if(checkbuffer[5]=='1'){
			    ...
			    while(1){	//每隔0.5秒发送一次请求信息	
				    printf("正在请求发送.........\n"); 
                    sendto(sock, "11111111", 8, 0, (SOCKADDR*)&PhyAddr1, sizeof(PhyAddr1));
                    memset(buffer, 0, sizeof(buffer));
                    recvfrom(sock, buffer, MAX_BUFFER_SIZE, 0, (SOCKADDR*)&PhyAddr1, &PhyLen1);
                    if(buffer[1]=='0'){
        	            printf("对方忙.....将稍后发送\n");
			            Sleep(500); 
		            }
			    ...
			    printf("发送的长度信息为：%s\n",buf);
			    printf("一共要发送%d帧\n",strlen(sendbuffer)/8);
			    sendto(sock, buf, strlen(buf), 0, (SOCKADDR*)&PhyAddr1, sizeof(PhyAddr1));
			    memset(buf,0,sizeof(buf));
			    recvfrom(sock, buf,MAX_BUFFER_SIZE, 0, (SOCKADDR*)&PhyAddr1, &PhyLen1);
			    if(buf[0]=='1'){
				    printf("对方接收成功！\n");
			    }
			    else{
                ...
                rep(i,0,strlen(sendbuffer)){//成帧发送
        	        ...
        	        if(cunt>=8){
        	            ...
        	            while(1){
        	                ...
        	                sendto(sock, TFrame, strlen(TFrame), 0, (SOCKADDR*)&PhyAddr1, sizeof(PhyAddr1));
        	                printf("发送完毕，正在等待确认.......\n");
        	                recvfrom(sock, checkbuffer,MAX_BUFFER_SIZE, 0, (SOCKADDR*)&PhyAddr1, &PhyLen1);
        	                if(checkbuffer[5]=='1'){
        	    	            printf("确认成功！第%d帧发送完毕\n",framenum);
        	    	            break;
			    	        }
		        ...
		        printf("发送完毕！一共发送%d帧，重发%d次\n",framenum,resendnum); 
			    printf("正在与交换机断开连接\n");
			    sendto(sock, mmmmmmm, strlen(mmmmmmm), 0, (SOCKADDR*)&PhyAddr1, sizeof(PhyAddr1));
			    printf("断开连接成功！\n");
            }
		}
        else{
			memset(sendbuffer, 0, sizeof(sendbuffer));
			sockaddr_in PhyAddr3;
            memset(&PhyAddr3, 0, sizeof(PhyAddr3));
            int PhyLen3=sizeof(PhyAddr3);
		    printf("\n\n---------正在从物理层接收信息-----------\n\n");
			while(1){//接收请求信息
			    memset(buf,0,sizeof(buf));
				recvfrom(sock, buf, MAX_BUFFER_SIZE, 0, (SOCKADDR*)&PhyAddr3, &PhyLen3);
				if(!strcmp(buf,Broadcast)){
					printf("即将接收一个群发信息！\n");
					memset(buffer,0,sizeof(buffer));
                    recvfrom(sock, buffer, MAX_BUFFER_SIZE, 0, (SOCKADDR*)&PhyAddr3, &PhyLen3);
					memset(sendbuffer,0,sizeof(sendbuffer));
                    int j=0;
					//提取有效信息
		            ...
		            if(randnum%2==0){//通过产生一个随机整数，然后通过奇偶性来决定是否可以接收信息
		                sendto(sock, "00000000", strlen("00000000"), 0, (SOCKADDR*)&PhyAddr3, sizeof(PhyAddr3));
		                printf("正忙....无法接收信息\n");
		            }
		            else{
		                sendto(sock, "11111111", strlen("11111111"), 0, (SOCKADDR*)&PhyAddr3, sizeof(PhyAddr3));
				        printf("可接受信息.......");
					    break;
		            }
				}    	   
			...
			memset(buffer, 0, sizeof(buffer));
            recvfrom(sock, buffer, MAX_BUFFER_SIZE, 0, (SOCKADDR*)&PhyAddr3, &PhyLen3);
			sendto(sock, "111111111", 9, 0, (SOCKADDR*)&PhyAddr3, sizeof(PhyAddr3));
			int j=0;
			memset(sendbuffer,0,sizeof(sendbuffer));
			rep(i,8,strlen(buffer)-9){
            sendbuffer[j++]=buffer[i];
				    ...
				    printf("正在接收第%d帧\n",i++);
       	            ...
       	            while(1){
                        ...
                        if(!strcmp(data,check)){
                            printf("校验成功！");
                            sendto(sock, "111111111", strlen("111111111"), 0, (SOCKADDR*)&PhyAddr3, sizeof(PhyAddr3));
                            strcat(sendbuffer,data);
                            break;
			            }
			            printf("校验失败！即将发送错误信息........\n");
			            sendto(sock, "000000000", strlen("000000000"), 0, (SOCKADDR*)&PhyAddr3, sizeof(PhyAddr3));
