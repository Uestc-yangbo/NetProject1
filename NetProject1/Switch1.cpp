            ...
            if(FD_ISSET(sock[i],&rfds)){//如果监测到有接受
                ...
                if(!strcmp(buffer,mybuf)){//与群发标志进行对比
                    ...
                    if(k==0){
                        ...
                        sendto(sock[4], buf, strlen(buf), 0, (SOCKADDR*)&PhyAddr[4], sizeof(PhyAddr[4]));
                        recvfrom(sock[4],buffer,MAX_BUFFER_SIZE,0,(sockaddr*)&PhyAddr[4],&PhyLen[4]);
                        printf("请求成功！\n");
                        while(1){  
                            memset(buffer,0,sizeof(buffer));
                            recvfrom(sock[i],buffer,MAX_BUFFER_SIZE,0,(sockaddr*)&PhyAddr[i],&PhyLen[i]);
                            ...
                            if(!strcmp(buffer,mmmmmmm)){
                                sendto(sock[4],buffer,strlen(buffer),0,(sockaddr*)&PhyAddr[4],sizeof(PhyAddr[4]));//与路由器断开连接
                                printf("已断开连接！\n");
                                break;
                            }
                            sendto(sock[4],buffer,strlen(buffer),0,(sockaddr*)&PhyAddr[4],sizeof(PhyAddr[4]));
                            memset(buffer,0,sizeof(buffer));
                            recvfrom(sock[4],buffer,MAX_BUFFER_SIZE,0,(sockaddr*)&PhyAddr[4],&PhyLen[4]);
