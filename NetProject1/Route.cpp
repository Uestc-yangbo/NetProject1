...
int UpdateRoutingList(int Swicthnum,int Port){
    rep(i,1,20){
        if(RoutingList[Swicthnum][i]!=0){//先判断该端口号是否存在，存在就退出
            if(RoutingList[Swicthnum][i]==Port){
                break;
            }
        }
        else{
            if(RoutingList[Swicthnum][i]==0){//添加端口号
        ...
        rep(i,1,2){//排队检测每个端口是否接受到了信息
            ...
            if(FD_ISSET(sock[i],&rfds)){//如果监测到有接受
                ...
                if(!strcmp(buffer,Broadcast)){
                    ...
                    UpdateRoutingList(i,SourcePort);//更新路由表
                    rep(j,1,2){//循环将信息发送至每个端口
                        if(j==i)
                        continue;
                        else{
                            sendto(sock[j], Broadcast, strlen(Broadcast), 0, (SOCKADDR*)&PhyAddr[j], sizeof(PhyAddr[j]));//先发送群发命令
                            sendto(sock[j], buf, strlen(buf), 0, (SOCKADDR*)&PhyAddr[j], sizeof(PhyAddr[j]));//发送群发信息
                        }
                    }
                    printf("已断开连接！\n");
                    continue;
