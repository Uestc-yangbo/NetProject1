...
char RecLangFrame[MAX_BUFFER_SIZE];//接收帧 
...
int FindFrame(char *str){
	int head=0;//找帧头 
	int hashead=0;//判断是否有帧头
	rep(i,0,strlen(str)){
		if(str[i]=='0')
		{
			if(str[i+1]=='1'&&str[i+2]=='1'&&str[i+3]=='1'&&str[i+4]=='1'&&str[i+5]=='1'&&str[i+6]=='1'&&str[i+7]=='0'){
				head=i;
                hashead=1;
				break;
			}
	...
	int end=0;
	int hasend=0;//判断是否有帧尾
	rep(i,head+8,strlen(str)){
		if(str[i]=='0')
		{
			if(str[i+1]=='1'&&str[i+2]=='1'&&str[i+3]=='1'&&str[i+4]=='1'&&str[i+5]=='1'&&str[i+6]=='1'&&str[i+7]=='0'){
				end=i+7;
...
int FindLangFrame(char *str){
	...
	int j=0;
	memset(RecLangFrame,0,sizeof(RecLangFrame));
	rep(i,head,end){//将找出来的帧拷贝到接收帧中
		RecLangFrame[j++]=str[i];
	}
	if(hashead&&hasend){
		return 1;
	}   
...
int main(){
    ...
    int PhyPort=11100,NetPort=11900;
    //网络层信息 
    printf("请设置网络层端口：\n");
	printf("%d\n",NetPort);
    sockaddr_in NetAddr;
